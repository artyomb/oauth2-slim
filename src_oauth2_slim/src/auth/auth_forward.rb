    ...
    base.class_eval do
      ...
      get '/auth' do
        ...
        if valid_token?(token)
          ...
          redirect "#{OAUTH_AUTH_URL}?client_id=your-client-id&redirect_uri=#{OAUTH_TOKEN_URL}&response_type=code&scope=openid+profile+email&state=#{URI.encode_www_form_component(full_uri)}", 302
        end
      end
