    ...
    base.class_eval do
      ...
      get '/auth' do
        ...
        if valid_token?(token)
          ...
          redirect "http://#{AUTH_DOMAIN}/authorize?client_id=your-client-id&redirect_uri=/_oauth&response_type=code&scope=openid+profile+email&state=#{URI.encode_www_form_component(full_uri)}", 302
        end
      end
