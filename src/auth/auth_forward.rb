      ...
      get '/auth' do
        ...
        if valid_token? token
          ...
          if parsed_params.key?('code') && AUTH_CODES.key?(parsed_params['code'])
            ...
            redirect "#{FORWARD_OAUTH_AUTH_URL}?client_id=your-client-id&redirect_uri=#{request.env['HTTP_REFERER']}&response_type=code&scope=openid+profile+email&state=#{URI.encode_www_form_component(request.env['HTTP_REFERER'])}", 302
          end
        end
      end
